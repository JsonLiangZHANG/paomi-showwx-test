...
stareal
    .controller("SeatController", function ($rootScope,$scope,$http,$compile,$interval,$stateParams,$location,$anchorScroll,$api, $sce, base64, $state, $alert,$timeout, localStorageService,FileUploader) {
        ...
        $("#price").on('touchend',"path",function(){
            ...
            if(a<2){
                // matrix.add(1,0,0,1,-((2/a-1)*(xchang+x0pointer-topx)+ecurrent/a),-((2/a-1)*(ychang+y0pointer-20)+fcurrent/a));
                // matrix.add(2/a,0,0,2/a,0,0); //放大倍数
                var ma0=matrix.translate((-ecurrent)*1/a, (-fcurrent)*1/a);
                var scale= matrix.scale(2/a, 2/a);
                var ma1=matrix.translate((-xchang), (-ychang));
                scalebei=3;
                g.animate({
                    transform:ma0.toString(),
                    transform:scale.toString(),//
                    transform:ma1.toString()
                }, 1500, mina.linear);
                // g.animate({
                //     transform: matrix.toString()	//
                // }, 1000, mina.linear);
            }else{
                matrix.add(1,0,0,1,-(x0pointer-topx+ecurrent)/a,-(y0pointer+fcurrent-20)/a);
                g.animate({
                    transform: matrix.toString()
                }, 1000, mina.linear);
        ...
        $scope.big=function(n){
            ...
            if(n==1&&a<2){
                ...
                if(scalebei==3){scalebei--;
                    ...
                    scalebei--;
                    matrix.add(1,0,0,1,-(ecurrent-topx)/a,-(fcurrent)/a);
        ...
        $scope.selectSeats=function(section_id,status,price_seat_id,id,event,section_name,row,columns,price) {
            if (status == '待售') {
